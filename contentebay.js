...
window.onload = () => {
  ...
  const elements = document.getElementsByClassName("s-item__sep");
...
const applyButtons = () => {
  var divs = document.querySelectorAll('[class="s-item__info clearfix"]');
  divs.forEach((div) => {
    var buttonElement =
    ...
    if (newDiv.innerHTML) {
      ...
      fetch(url) //Get request of the generated url
        ...
        .then((text) => {
          ...
          //  '[class="dangerous-html mb3"]'
          // );
         
           var items = doc.querySelectorAll(
            '[class="vim x-about-this-item"]'
...
//document.getElementById('add_buttons').addEventListener("click", function(){ console.log("clicked"); applyButtons(); });
